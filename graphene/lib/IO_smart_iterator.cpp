...
{
	//std::cout << "go into : it->load_key(level);" << std::endl;
	double blk_tm = wtime();
	while(cd->circ_free_chunk->is_empty()){/*std::cout<<"while";*/}
	this->wait_comp_time += (wtime() - blk_tm);

	// std::cout << "First get_chunk." << std::endl;
	// cd->get_chunk();
	// std::cout << "First load_chunk." << std::endl;
	cd->load_chunk();
	// std::cout << "Second  get_chunk." << std::endl;
	cd->get_chunk();
	// std::cout << "Second load_chunk." << std::endl;
	// cd->load_chunk();
/*	std::cout  << "cd->circ_load_chunk->is_empty()" << cd->circ_load_chunk->is_empty() << "\t"
				<< "cd->circ_free_chunk->is_full()" << cd->circ_free_chunk->is_full() << "\t"
				<< "cd->circ_free_ctx->is_full() " << cd->circ_free_ctx->is_full()<< "\t"
				<< "this->reqt_blk_count" << this->reqt_blk_count << std::endl;*/
